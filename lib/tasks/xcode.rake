...
namespace :xcode do
  ...
  task :command_line_utilities do
    if File.directory?('/Library/Developer/CommandLineTools')
      ...
      begin
        ...
        name = out.scan(/^\s*\*\s*(?:Label:)?\s*(Command Line Tools.*)\s+?$/).flatten.last

        raise "Command Line Tools are not available for download:\n#{out}" unless name
  ...
  task :accept_license => [:mas] do
    installed = begin
      capture('xcode-select', '--print-path').include?('/Xcode.app/')
    rescue NonZeroExit
      false
    end
